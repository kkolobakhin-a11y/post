...
"""
NULL — Channel 0 (Пустота) Autoposter (fixed)
- Поддержка ключей: "atmo" или "atmosphere"
- Поддержка плейсхолдеров в CTA: "{link}" или "[BOT_LINK]"
"""
...
def load_templates(path: str):
    ...
    # Нормализация ключей
    if "atmo" not in data and "atmosphere" in data:
        data["atmo"] = data["atmosphere"]
...
def render_text(slot: str, phrase: str, bot_link: str | None) -> str:
    ...
    if slot == "cta":
        link = bot_link or ""
        return phrase.replace("{link}", link).replace("[BOT_LINK]", link)
    return phrase
